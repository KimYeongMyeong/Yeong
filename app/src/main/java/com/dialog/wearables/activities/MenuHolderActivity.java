...
//                double datetimes = (currentDatetime / 1000) + (0.001 * (currentDatetime % 1000));
                try {
                    ...
                    if (!GlobalVar.accArr.isAccDataXEmpty()) {
                        jsonData.put("acc_x", GlobalVar.accArr.popAccDataX());
                    }
                    if (!GlobalVar.accArr.isAccDataYEmpty()) {
                        jsonData.put("acc_y", GlobalVar.accArr.popAccDataY());
                    }
                    if (!GlobalVar.accArr.isAccDataZEmpty()) {
                        jsonData.put("acc_z", GlobalVar.accArr.popAccDataZ());
                    }

                    if (!GlobalVar.gyroEulerArr.isEulerDataXEmpty()) {
                        jsonData.put("roll", GlobalVar.gyroEulerArr.popEulerDataX());
                    }
                    if (!GlobalVar.gyroEulerArr.isEulerDataYEmpty()) {
                        jsonData.put("pitch", GlobalVar.gyroEulerArr.popEulerDataY());
                    }
                    if (!GlobalVar.gyroEulerArr.isEulerDataZEmpty()) {
                        jsonData.put("yaw", GlobalVar.gyroEulerArr.popEulerDataZ());
                    }
//                    //Log.d(TAG, "popAccDataX(): "+ GlobalVar.accArr.popAccDataX());
//                    jsonData.put("roll", GlobalVar.gyroEulerArr.popEulerDataX());
//                    jsonData.put("pitch", GlobalVar.gyroEulerArr.popEulerDataY());
//                    jsonData.put("yaw", GlobalVar.gyroEulerArr.popEulerDataZ());
